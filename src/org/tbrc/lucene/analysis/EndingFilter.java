...
public class EndingFilter extends TokenFilter {
	static char APOSTROPHE = '\'';

	public EndingFilter(TokenStream in) {
		super(in);
	}
	...
	public final boolean incrementToken() throws java.io.IOException {
		...
		if (len > 3) {
			if (buffer[len - 3] == APOSTROPHE && buffer[len - 1] == 'i' && buffer[len - 1] == 's') {
		...
		if (len > 2) {
			if (buffer[len - 2] == APOSTROPHE && (buffer[len - 1] == 'i' || buffer[len - 1] == 'o')) {
